 ...
 * @module db/repositories/company
 */
 ...
 * @property {number} deleted_by - Deleter user ID
 */
...
export class CompanyRepository extends BaseRepository {
  ...
  async findByUUID(uuid) {
    const sql = `SELECT * FROM ${this.tableName} WHERE uuid = ? AND deleted_at IS NULL`;
    return this.pool.get(sql, [uuid]);
  }
   ...
   * @param {string} [excludeUUID] - UUID to exclude (for updates)
   * @returns {Promise<boolean>}
   */
  async isSlugAvailable(slug, excludeUUID = null) {
 ...
 * @property {number} invited_by - Inviter user ID
